...
- 각 자동차에 이름을 부여할 수 있다.
  - 자동차 이름은 쉼표로 구분해서 입력받는다.
- 자동차 경기를 일정횟수 실행하고, 우승자(혹은 우승자들)를 출력한다.
  - 실행할 경기횟수를 숫자로 입력받는다.
  - 각 자동차는 무작위로 전진여부를 결정한다.
  - 각 경주가 끝날때마다, 각 자동차 이름과 전진여부를 출력한다.
  - 모든 경주가 끝나면 우승자를 출력한다. 둘 이상이면 쉼표로 구분해서 모두 출력한다.
- 오류처리. 잘못된 경우 alert를 통해 메시지를 보여주고, 다시 입력할 수 있게 한다.
  - 자동차 이름은 5자 이하만 가능하다.
  - 경기횟수는 숫자만 받을 수 있다.
